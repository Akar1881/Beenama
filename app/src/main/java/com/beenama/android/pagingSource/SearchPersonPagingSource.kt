...
package com.beenama.android.pagingSource
...
class SearchPersonPagingSource(
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, JSONObject> {
        return try {
            ...
            val response = fetchSearchResultsFromApi(page, query)
    ...
    private suspend fun fetchSearchResultsFromApi(page: Int, query: String): String? {
        return withContext(Dispatchers.IO) {
            try {
                val url = URL("https://api.themoviedb.org/3/search/person?query=$query&page=$page&api_key=$apiKey${BaseActivity.getLanguageParameter(context)}")
