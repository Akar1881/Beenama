// 4KHDHub Scraper for Node.js
// Converted from Nuvio Local Scrapers format
...
function findBestMatch(results, query, targetYear) {
  ...
  var scored = results.map(function (r) {
    ...
    if (ty && !isNaN(ty)) {
      if (rYear === ty) score += 200;
      else if (rYear && Math.abs(rYear - ty) <= 1) score += 30;
      else if (rYear) score -= 100;
    }
...
// Convert download links to stream entries
...
function getStreams(tmdbId, type, season, episode) {
  ...
  function finalizeToStreams(links) {
    var tasks = links.map(function (link) { return validateVideoUrl(link.url); });
    ...
    return searchContent(tmdb.title + (tmdb.year ? (' ' + tmdb.year) : '')).then(function (results) {
      ...
      return loadContent(best.url).then(function (content) {
        ...
        return Promise.all(resolverTasks).then(function (resolvedArr) {
          ...
          resolvedUrlsCache[cacheKey] = resolved;
